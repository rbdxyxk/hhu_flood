...
public class AutoCalServiceImpl  implements AutoCalService{
    @Autowired
    ...
    @Autowired
    AnaResultMapper anaResultMapper;

    private String achId;

    @Override
    public AutoCalInitParams AutoOninit(){
        AutoCalInitParams result = new AutoCalInitParams();
        ...
        String name;
        name=result.getChecked();
        if(name.contains("当前运行方案")){
            name=name.substring(0,name.length()-8);
        }
        result.setParaForecastNow(name);
        //2.获取降雨数据
        ...
        result.setRname(LocalDateTime2String(LocalDateTime.now())+"自动预报成果"+"(Maker)"+LocalDateTime.now().getHour()+LocalDateTime.now().getMinute()+LocalDateTime.now().getSecond());
        result.setCtime(LocalDateTime2String(LocalDateTime.now()));
        return result;
    }
    ...
    public String saveborderPara(AutoCalInitParams initParams) {
        achId="SLHRES"+LocalDateTime2String(LocalDateTime.now())+"自动预报";
...
//        BigDecimal flowDeep=initParams.getP9();
        ...
        for (int i = 0; i < 5; i++)
        {
            ...
            rTmp=paraWeatherMapper.insertParaWeather(achId,CalRegCD,now,RainHour[i],RainHour[i].multiply(tf), RainHour[i].multiply(tf),RainHour[i].multiply(tf),BigDecimal.valueOf(3),BigDecimal.valueOf(75),BigDecimal.valueOf(300));
        ...
        achNm = LocalDate2String(LocalDate.now() )+ "自动预报成果" + "(Maker)"+now.getHour()+now.getMinute()+now.getSecond();
        calBgTime=LocalDateTime2String(initParams.getDate().atTime(initParams.getHour(),0,0));
        infectRow+=paraCalplanMapper.insertCalplan(achId,achNm,initParams.getChecked(),calBgTime,now, BigDecimal.valueOf(1));

        infectRow+=anaResultMapper.insert0(achId.substring(0,achId.length()-4));
