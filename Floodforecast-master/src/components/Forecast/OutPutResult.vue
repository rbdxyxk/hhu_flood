<template>
  <div v-if="this.justShow">

    <div style="padding-left: 50px">
      <h3><span class="label label-default">成果编辑与输出</span></h3>
    </div>
    ...
    <div id="table1" style="border: 1px black solid;width: 1000px;margin: auto;padding-left: 15px;margin-top: 10px;">
      ...
      <div id="AutoOrMan" style="display: inline-block;width: 43%;margin-left: 30px">
        ...
        <div class="dropdown">
          ...
          <div class="inlineDiv">
            <input type="radio" id="btnDownMenu1" v-model="selectMenu" @change="getOtherData" name="selectMenu"
        ...
        <div class="dropdown">
          <div class="inlineDiv">
            <input type="radio" id="btnDownMenu2" v-model="selectMenu" @change="getOtherData" name="selectMenu"
                   value="man"/>
      ...
      <div id="selectRuOrShui" style="display: inline-block;width: 48%;padding-top: 20px;margin-left: 30px">
        ...
        <div class="radio">
          <label>
            <input type="radio" id="select1" v-model="selectRuOrShuiOrFenqu" name="selectRuOrShuiOrFenqu" value="ru"
                   checked="checked"/>
            入库流量、水量过程
          </label>
        </div>
      ...
      <div id="btns" style="display: inline-block;width: 46%;margin-top: 30px;float: right">
        ...
        <button class="btn btn-default" @click="deleteResult">删除当前成果</button>
      </div>

    </div>
    <div id="table2" style="margin: auto;width: 50%;padding-top: 20px;padding-bottom: 20px;text-align: center">
      <!--    <table class="table table-striped table-bordered table-hover" id="table_tr">-->
      <!--        <tr>-->
      <!--            <td>计算时段</td>-->
      <!--            <td>入库时间</td>-->
      <!--            <td>入库流量</td>-->
...
export default {
  ...
  methods: {
    deleteResult(){
      ...
      axios.post('http://localhost:8083/anaResult/delete?id=' + this.id)
        .then(res => {
          console.log('删除成功')
          this.$bus.$emit('refreshComponent')
        });
    },
        ...
        .then(res => {
          ...
          if (this.selectedStatus == 1) {
            ...
            for (let i = 0; i < data.length; i++) {
              ...
              data1[i] = parseInt(data[i].v);
              data2[i] = parseInt(data[i].p);

              data5[i] = data[i].v.toFixed(2);
              data6[i] = data[i].p;
    ...
    setTimeout(() => {
      ...
      window.edit = function (obj) {
        ...
        var new_str = '            <td></td>\n' +
          ...
          '            <td><button class="btn btn-default" onclick="updateData()">修改</button></td>\n' +
      ...
      let data33 = this.data3
      window.updateData = function () {
        let id = that.id;
        console.log("id = " + id);
        console.log('updateData...' + id + '==行...' + rowIndex)
        ...
        axios.post('http://localhost:8083/anaResult/edit?' +
          'id=' + id +
          '&flowtm=' + data33[rowIndex - 1] +
          '&data1=' + data1 +
          '&data2=' + data2 +
          '&status=' + status
        )
       ...
       * 删除某行
       */
      window.delete_row = function (obj) {
      ...
      window.deleteData = function (obj) {
        ...
        console.log('deleteData...' + id + 'test....' + text)

        axios.post('http://localhost:8083/anaResult/delete?' +
          'id=' + id +
          '&flowtm=' + text
        ).then(res => {
          console.log("删除成功")
